...
def count_CWRF_on_MSWEP_percentiles(PERCENTILE_FOLDER, FREQ_FOLDER, INTENSITY_FOLDER, config, file, percentiles):
    # get percentile files
    percentile_title = "MSWEP"
    per_files = [f for f in os.listdir(PERCENTILE_FOLDER) if f.startswith(percentile_title)]
    # get expected shape of  percentile files
    ...
    # time is days since 0001-01-01
    ref_time = datetime.datetime(1,1,1)
    season = {1:0, 2:0, 3:1, 4:1, 5:1, 6:2, 7:2, 8:2, 9:3, 10:3, 11:3, 12:0}
    ds = netCDF4.Dataset(file,mode='r')
    times = ds.variables["time"]
    times = np.ma.masked_invalid(times)

    dt = ref_time + datetime.timedelta(days=times[0])
    cur_season = season[dt.month]
        ...
        try:
            ...
            if season[dt.month] != cur_season:
                for p, percentile in enumerate(percentiles):
                    percentile_str = percentile_title + str(percentile).replace(".","-")
                    # frequency path
                    f_name = f"{dt.year}_{season_labels[cur_season]}_freq.nc"
                    f_path = f"./{FREQ_FOLDER}/{percentile_str}/"
                    if not os.path.exists(f_path):
                        os.makedirs(f_path)
                    try:
                        save_NETCDF(lon2d, lat2d, frequency_count[:,:,p], 
                            "EX-PR-FRQ", 
                            "exceedences/season", 
                            f"Top {percentile_str} Extreme Precip Freq in CWRF", 
                            f_path, f_name, 
                            f"{season_labels[cur_season]} Freq of Top {percentile_str} Percentile Extreme Precipitation in CWRF"
                        )
                    except Exception as e:
                        print(f"Error saving {f_name}")
                        print(e)
                    ...
                    # intensity path
                    i_name = f"{dt.year}_{season_labels[cur_season]}_avg-intensity.nc"
                    i_path = f"./{INTENSITY_FOLDER}/{percentile_str}/"
                    if not os.path.exists(i_path):
                        os.makedirs(i_path)
                    try:
                        ...
                        save_NETCDF(lon2d, lat2d, intens_data, 
                            "EX-PR-INTENS", 
                            "mm/day", 
                            f"Top {percentile_str} Extreme Precip Average Intensity in CWRF", 
                            i_path, i_name, 
                            f"{season_labels[cur_season]} Avg Intensity of Top {percentile_str} Percentile Extreme Precipitation in CWRF"
                        )
                    except Exception as e:
                        print(f"Error saving {i_name}")
                        print(e)
            ...
            if init:
                ...
                print(f"One file took {end - start} seconds")
                print(f"All files estimated to take {len(times) * (end - start)} seconds")
                init = False
...
def count_CWRF_on_DMET_percentiles(PERCENTILE_FOLDER, FREQ_FOLDER, INTENSITY_FOLDER, config, file, percentiles):
    # get percentile files
    percentile_title = "DMET"
    per_files = [f for f in os.listdir(PERCENTILE_FOLDER) if f.startswith(percentile_title)]
    # get expected shape of  percentile files
